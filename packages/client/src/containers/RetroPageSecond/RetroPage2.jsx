...
function RetroPage2() {
  ...
  const handleAddComment = async (questionId) => {
    ...
    setComments(prevComments => ({
      ...prevComments,
      [questionId]: [...(prevComments[questionId] || []), { id: newCommentId, text: commentText }]
    }));
  ...
  const handleJoin = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch retro session: ${response.statusText}`);
      }
      ...
      alert('Failed to join the retro session. Please check the code and try again.');
    }
  }, [joinCode]);
  ...
  const handleCommentInput = (questionId, e) => {
    if (e.key === 'Enter') {
      ...
      setInputValues(prevValues => ({ ...prevValues, [questionId]: e.target.value }));
    }
  };
  ...
  return (
    <div>
      ...
      <h2>Retro</h2>
      <button className="newRetro-button" type='button' onClick={handleNewRetro}>New Retro</button>
      </div>
      {retroCode && <p>Retro code: {retroCode}</p>}
      <input className='joinCodeInput'
        type="text"
        placeholder="Enter code to join"
        value={joinCode}
        onChange={(e) => setJoinCode(e.target.value)}
      />
      <button className='joinButton' type='button' onClick={handleJoin}>Join</button>
    </div>
     {selectedQuestions.map(question => (
        <div key={question.id} className="container">
          <div className="question">{question.text}
          <div className="box">
            {comments[question.id] && comments[question.id].map(comment => (
            ...
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && handleCommentInput(question.id, e)}
              className="commentInput"
            />
          
            <button type="button" onClick={() => handleAddComment(question.id)} className="addCommentButton">
              +
            </button>
          </div>
         </div>
      ...
      <button className='completeButton' type='button' onClick={handleComplete}>Complete</button>
    </div>
  );
}
