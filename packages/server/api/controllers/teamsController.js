...
const createTeam = async (body) => {
  ...
  const insertedMembers = (await knex('Employees')
    .whereIn('email', newEmployees.map((employee) => employee.email))
    .select('id'))
    .map(newMember => newMember.id)


  // Create team members
  const teamMembersPayload = [...insertedMembers, ...existingEmployees].map(employeeId => ({
